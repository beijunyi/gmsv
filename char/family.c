...
void FAMILY_SetPoint(int fd, int meindex, char *message)
{
   ...
   for (i = 0; i < arraysizeof(fmdplevelexp); i++){
	...
	int iFmIndex = CHAR_getWorkInt(meindex,CHAR_WORKFMINDEXI);
	for( i=0; i<FAMILY_MAXNUM; i++){
   ...
   if ((fmlevel < MINFMLEVLEFORPOINT) || (fmdptop.fmtopdp[iFmIndex]<30000)){//3W家族声望 or 人数小於30人
	lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
		WINDOW_BUTTONTYPE_OK,
		-1, -1,
