...
BOOL MAP_readMapConfFile( char* filename )
{
    ...
    while( fgets( line , sizeof( line ) , file ) ){
        ...
        for( i = 0 ; i < MAP_DATAINT_NUM + MAP_DATACHAR_NUM ; i ++ ){
            ret1 = getStringFromIndexWithDelim(line, " " ,i + offset, token, sizeof(token) );
            if( ret1 == FALSE )continue;
...
BOOL MAP_readBattleMapConfFile( char* filename )
{
    ...
    while( fgets( line , sizeof( line ) , file ) ){
		...
		if( line[0] == '$' ){
			...
			if( iRet < 1 ){
				print( "!!!!!ERROR 无法读取战斗地图(%s)( line %d )\n", filename, linenum );
			}
			for( i = 0; i < iRet; i ++ ){
				BattleMapNo[i] = iPre[i];
				if( BattleMapNo[i] < 0 ){
					print( "!!!!!ERROR 战斗地图号码为负值 (%s)( line %d )\n", filename, linenum );
				}
			}
		...
		for( i = iFirst; i <= iLast; i ++ ){
			if( MAP_getImageInt( i, MAP_SETED_BATTLEMAP ) > 0 ){
				print( "!!!!!Warning 相同金钱重复设定了两次(%s)( line %d )(%d)(%d & %d)\n",
					filename, linenum, i,
...
//#define MAX_MAP_FILES 1200 // 地图目录最大档案数
...
BOOL MAP_readMapDir( char*  dirname )
{
    int     mapfilenum=0;
    STRING64 *filenames;
    filenames = (STRING64 *)allocateMemory( sizeof( STRING64 ) * MAX_MAP_FILES );
    int     filenum;
    int     i;
		if( filenames == NULL ){
	    print( "Reading map Create files... Memory Error\n" );
			return FALSE;
		}
    filenum = rgetFileName( dirname, filenames, MAX_MAP_FILES );
    if( filenum == -1 ){
        fprint( "Can't get filenames under %s directory\n" , dirname );
    ...
    for( i = 0 ; i < filenum ; i ++ ){
        if( MAP_IsMapFile( filenames[i].string ) )mapfilenum++;
    }
