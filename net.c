...
{
	//这里修正一个不知道是什么BUG的BUG，堆栈溢出？fd不知道怎么被修改了.....下面是调试信息
/*	ok!1:-1 2:-1
...
//    static unsigned int nu_time=0;
	...
	//获得连接模式 acceptmore=1 正常连接 acceptmore=0 服务器繁忙？ acceptmore=-1 GM下关闭GMSV指令
...
MTIO_loop( void )
{
   ...
   //往writebuffer缓冲区写数据大小 单位字节
    ...
    /* 处理客户端连接请求 */
		...
		{
			gettimeofday( &et, NULL );
      if (time_diff_us(et,st) >= looptime_us)//执行一次netloop的时间
      	{
...
MUTLITHREAD BOOL netloop_faster( void )
{
    ...
    /*if( faster_display_interval != time(NULL )){
      ...
      if (time_diff_us(tms,tmv) >= looptime_us)//执行一次netloop的时间
      	{
          ...
          break;
				default:				//正常的连接模式
				{
					...
					if( i_timeNu != time(NULL) ) // 每整秒执行一次
					{
						i_timeNu = time(NULL);
#ifdef _DEL_DROP_GOLD
						if( total_count % jzGolddeltime == 0 ) { //每分钟执行
							GOLD_DeleteTimeCheckLoop();
							total_count=0;
						}
#endif
	...
	if( f ){
		...
		if( strcmp(message,"") == 0
			...
			if( fgets(message2, sizeof(message2), f) ){
				print("\nReceived Shutdown signal...n分钟後维修\n\n");
